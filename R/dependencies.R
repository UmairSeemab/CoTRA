...
# Runtime dependency types only.
# Avoid installing Suggests because they greatly increase the dependency tree
# and are not required for normal CoTRA runtime operation.
cotra_runtime_dependency_types <- function() {
...
#'
#' Installs CRAN, Bioconductor, and selected GitHub dependencies required
#' by the CoTRA app.
#'
...
install_cotra_dependencies <- function(ask = interactive(), update = FALSE) {
  ...
  # Final verification
  # ----------------------------------------------------------
  result <- check_cotra_dependencies(quiet = FALSE)

  if (!isTRUE(result$ok)) {
    stop(
      "CoTRA dependency installation is incomplete. Missing: ",
